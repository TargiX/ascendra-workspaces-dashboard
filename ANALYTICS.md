...
Only `ascendra-workspaces-dashboard-sand.vercel.app` collects pageviews and page leaves. Preview/local hosts and Do Not Track visitors are excluded. Events carry `app_id=ascendra-workspaces-dashboard` and `environment=production`; anonymous IDs and persistence are site-specific. No session replay, autocapture, form values, wallet IDs, or chat content are collected. URL queries and fragments are removed; standard campaign attribution remains available.
